...
export async function POST(request: Request) {
  try {
    ...
    const { error, value } = createEmployeeSchema.validate(body, { abortEarly: false });
    ...
    if (existingError) {
      return NextResponse.json({ existingError: existingError.message }, { status: 500 });
    }
    ...
    const { data, error: supabaseError } = await supabase
      .from('employee')
      .insert({ ...value, isActive: true }).select();

    console.log(data, "create staff")
    if (supabaseError)
      return NextResponse.json({ supabaseError: supabaseError.message }, { status: 500 });
